...
def redact_company():
    with SessionLocal() as session:
        ...
        with tabs[1]:
            ...
            def _company_label(comp: company.Company) -> str:
                inn = (comp.inn or "").strip() or "—"
                account = clean_account(getattr(comp, "settlement_account", None)) or "—"
                return f"{comp.name} • ИНН {inn} • счёт {account}"
                ...
                [
                    {
                        "Название": c.name,
                        "ИНН": c.inn,
                        "Расчетный счет": clean_account(getattr(c, "settlement_account", None)) or "",
            ...
            with cL.popover("✏️ Редактировать компанию"):
                if not comp_list:
                    ...
                    search_key = "edit_comp_search"
                    selected_comp_key = "edit_comp_selected_company_id"
                    pending_comp_key = "edit_comp_selected_company_id_pending"

                    comp_search = st.text_input(
                        "Поиск компании",
                        key=search_key,
                        placeholder="Название, ИНН, счёт, управляющая...",
                    ).strip().lower()

                    filtered_companies = comp_list
                    if comp_search:
                        filtered_companies = [
                            c for c in comp_list
                            if comp_search in " ".join(
                                [
                                    (c.name or "").lower(),
                                    (c.inn or "").lower(),
                                    (clean_account(getattr(c, "settlement_account", None)) or "").lower(),
                                    (up_name_by_id.get(c.up_company_id, "") or "").lower(),
                                ]
                            )
                        ]

                    if not filtered_companies:
                        st.info("По текущему поиску компании не найдены.")
                        sel_c = None
                    else:
                        comp_ids = [c.id for c in filtered_companies]
                        comp_label_by_id = {c.id: _company_label(c) for c in filtered_companies}
                        pending_selected_id = st.session_state.pop(pending_comp_key, None)
                        if pending_selected_id in comp_ids:
                            st.session_state[selected_comp_key] = pending_selected_id
                        elif st.session_state.get(selected_comp_key) not in comp_ids:
                            st.session_state[selected_comp_key] = comp_ids[0]

                        def _shift_company(step: int) -> None:
                            current_id = st.session_state.get(selected_comp_key, comp_ids[0])
                            try:
                                current_idx = comp_ids.index(current_id)
                            except ValueError:
                                current_idx = 0
                            next_idx = max(0, min(len(comp_ids) - 1, current_idx + step))
                            st.session_state[selected_comp_key] = comp_ids[next_idx]

                        current_selected_id = st.session_state.get(selected_comp_key, comp_ids[0])
                        current_idx = comp_ids.index(current_selected_id)
                        nav_prev_col, nav_next_col = st.columns([1, 1])
                        nav_prev_col.button(
                            "←",
                            key="edit_comp_prev_btn",
                            disabled=current_idx == 0,
                            help="Предыдущая компания",
                            on_click=_shift_company,
                            args=(-1,),
                        )
                        nav_next_col.button(
                            "→",
                            key="edit_comp_next_btn",
                            disabled=current_idx == len(comp_ids) - 1,
                            help="Следующая компания",
                            on_click=_shift_company,
                            args=(1,),
                        )
                        ...
                        selected_company_id = st.selectbox(
                            "Компания",
                            options=comp_ids,
                            key=selected_comp_key,
                            format_func=lambda cid: comp_label_by_id.get(cid, "—"),
                        )
                        st.caption(f"Компания {comp_ids.index(selected_company_id) + 1} из {len(comp_ids)}")
                        sel_c = next((c for c in filtered_companies if c.id == selected_company_id), None)

                    if sel_c:
                        widget_suffix = f"_{sel_c.id}"
                        new_c_name = st.text_input("Название", value=sel_c.name, key=f"edit_comp_name{widget_suffix}")
                        new_c_inn = st.text_input("ИНН", value=sel_c.inn or "", key=f"edit_comp_inn{widget_suffix}")
                        new_c_account = st.text_input(
                            "Расчетный счет",
                            value=clean_account(getattr(sel_c, "settlement_account", None)),
                            key=f"edit_comp_account{widget_suffix}",
                        )
                        ...
                        new_up_name = st.selectbox(
                            ...
                            key=f"edit_comp_up{widget_suffix}",
                        )
                        new_primary = st.checkbox(
                            "Основная по умолчанию",
                            value=bool(getattr(sel_c, "is_primary", False)),
                            key=f"edit_comp_primary{widget_suffix}",
                        )

                        b1, b2 = st.columns(2)

                        if b1.button("Сохранить", key=f"save_comp_btn{widget_suffix}"):
                            try:
                                normalized_account = clean_account(new_c_account) or None
                                if normalized_account:
                                    account_exists = (
                                        session.query(company.Company)
                                        .filter(
                                            company.Company.settlement_account == normalized_account,
                                            company.Company.id != sel_c.id,
                                        )
                                        .first()
                                    )
                                    if account_exists:
                                        st.error("Компания с таким расчетным счетом уже существует.")
                                    else:
                                        sel_c.name = new_c_name.strip()
                                        sel_c.inn = (new_c_inn or "").strip()
                                        sel_c.settlement_account = normalized_account
                                        ...
                                        session.add(sel_c)
                                        session.commit()
                                        st.session_state[pending_comp_key] = sel_c.id
                                        st.success("Компания обновлена.")
                                        st.rerun()
                                else:
                                    sel_c.name = new_c_name.strip()
                                    sel_c.inn = (new_c_inn or "").strip()
                                    sel_c.settlement_account = None
                                    uc = next((u for u in up_all if u.name == new_up_name), None) if new_up_name else None
                                    sel_c.up_company_id = uc.id if uc else None
                                    sel_c.is_primary = bool(new_primary)
                                    session.add(sel_c)
                        ...
                        if b2.button("Удалить", key=f"del_comp_btn{widget_suffix}"):
                            try:
                                next_company_id = None
                                if filtered_companies:
                                    filtered_ids = [c.id for c in filtered_companies]
                                    if sel_c.id in filtered_ids:
                                        del_idx = filtered_ids.index(sel_c.id)
                                        remaining_ids = [cid for cid in filtered_ids if cid != sel_c.id]
                                        if remaining_ids:
                                            next_company_id = remaining_ids[min(del_idx, len(remaining_ids) - 1)]
                                session.delete(sel_c)
                                session.commit()
                                if next_company_id is not None:
                                    st.session_state[pending_comp_key] = next_company_id
                                else:
                                    st.session_state.pop(selected_comp_key, None)
            ...
            with cR.popover("➕ Добавить компанию"):
                with st.form("form_add_company"):
                    ...
                    c_account = st.text_input("Расчетный счет")
                    ...
                    if submit_c:
                        if not c_name.strip():
                            ...
                            try:
                                normalized_account = clean_account(c_account) or None
                                if normalized_account:
                                    account_exists = (
                                        session.query(company.Company)
                                        .filter(company.Company.settlement_account == normalized_account)
                                        .first()
                                    )
                                    if account_exists:
                                        st.error("Компания с таким расчетным счетом уже существует.")
                                        ...
                                        obj = company.Company(
                                            name=c_name.strip(),
                                            inn=c_inn.strip(),
                                            settlement_account=normalized_account,
