...
def _report_year_from_rm_or_date(report_month: str | None, date_val) -> int | None:
    if report_month:
        raw = str(report_month).strip()
        if _is_rm_yyyy_mm(raw) and raw[:4].isdigit():
            return int(raw[:4])
        parts = raw.split()
        if len(parts) >= 2 and parts[-1].isdigit():
            return int(parts[-1])
    try:
        return int(date_val.year) if date_val else None
    except Exception:
        return None
...
def import_edit_operations_tab():
    ...
    with SessionLocal() as session:
        ...
        gb.configure_default_column(editable=False, filter=True, resizable=True, minWidth=140)  # всё read-only по умолчанию
        gb.configure_selection(selection_mode="multiple", use_checkbox=True)
        gb.configure_grid_options(alwaysShowHorizontalScroll=True, suppressHorizontalScroll=False)
        ...
        if st.button("💾 Сохранить изменения из таблицы (месяц/категория)", use_container_width=True):
            try:
                ...
                for _, row in edited_df.iterrows():
                    ...
                    if new_rm:
                        new_rm = (
                            month_label_to_value.get(new_rm)
                            or rm_from_ru_label(new_rm)
                            or new_rm
                        )
        ...
        with col3:
            btn_assign_groups = st.button("Массовое начисление (по категориям)", use_container_width=True)
        ...
        if btn_transfer_sel:
            ...
            for _, row in selected.iterrows():
                ...
                if report_month_val:
                    report_month_val = (
                        month_label_to_value.get(report_month_val)
                        or rm_from_ru_label(report_month_val)
                        or report_month_val
                    )

                date_from_row = pd.to_datetime(row.get("Дата"), dayfirst=True) if row.get("Дата") else None
                report_year_val = _report_year_from_rm_or_date(report_month_val, date_from_row or (op_db.date if op_db else None))

                stmt = statement.Statement(
                    row_id=row.get("row_id"),
                    date=date_from_row if date_from_row is not None else (op_db.date if op_db else None),
